...
> **Status (May 2026)**: v3 ✅ complete + v3.5 chunks 5 / 6 (a/b/c) / 7 / 8a / **8a-ext** / **8a-ext V2** / 9 / 10 / 11 / **14** ✅ + UX-001/UX-002/UX-003/**UX-004**/**UX-005**/**UX-007** ✅ + hotfix-3 ~ 10 ✅ shipped. **UX-004 tool 调用过渡语**:LLM 调工具前先输出 "嗯让我看看 / 等我查一下" 6-15 字过渡语 + frontend ChatInput 显示 ``查日历…`` / ``查歌单…`` / etc 文案的 loading pill(基于 tool_name 前缀 mapping),治"问完后 30 秒沉默 app 卡死"体感。Backend 新增 ``tool_use_start`` / ``tool_use_done`` WS event(后者带 ``duration_ms`` 字段为未来 "工具好慢哦" feedback UI 留口子)。**Tech debt**(用户额外约束记录):TTS 过渡语真出声留 chunk 15 / UX-006(需 sentence-by-sentence streaming 架构改);character-specific 过渡语示例留 chunk 12 persona 加厚同期。 **chunk 14 activity timeline**: 跟 chat_history 平行的第二条 timeline 持久化每日 app/URL session,Momo 能在对话中引用今日活动(`## 用户今日活动 - VS Code 3小时 / 终端 1小时 / Chrome (主要看 jobs.bilibili.com 招聘 95分钟) / ...`)。三层:实时 watcher (chunk 8a/8a-ext)→ 持久化 session 写入 (chunk 14 poll listener) → LLM context 注入 + ToolRegistry capability + Timeline drawer 浏览 + 每日 cleanup 保 30 天。隐私五道闸(数据源黑名单 / 写入层 defensive 二过 / capability double idle 排除 / UI 删除 + 强制 date 参数防误删 / 全本地 SQLite 不上传)。 Memory 三层结构(chunk 9 + chunk 10 worker + chunk 11 profile_data)。chunk 8a 简化屏幕感知 + **chunk 8a-ext 智能陪伴 judge 慢路径** —— 在快路径(IDE/音乐/技术文档)未命中时,用 qwen-turbo 判断用户停 5+ min 的 app/URL 是否值得主动 chime in,yes 走 ChatAgent 主对话,no 静默。三重门(min_stay 5 min + judge throttle 10 min + fire throttle 30 min)+ 共享 daily_cap (5/天) 防滥用。topic_hint 作 anchor 注入主 LLM 让 Momo 知道往哪个方向搭话。**chunk 8a-ext V2 加 idle 第 4 道闸** —— ``ioreg -c IOHIDSystem`` 抽 ``HIDIdleTime`` 纳秒转秒,键鼠静止 > 300s 视为离开电脑,judge 直接 skip 不烧 LLM 不"自言自语"。零新 pip 包(复用 chunk 8a subprocess 范式)+ 跨平台 graceful(非 macOS 返 None 保持 V1 行为)。SettingsPanel 二级 toggle + 三级 idle 阈值输入(0..3600s,0 = 关闸),默认 ON 可关。UX-001/UX-002/UX-003 SettingsPanel 三层 accordion 全栈完成 + 情绪 UI 左上角。hotfix-3..hotfix-8 治本 strip / TTS / config duplicate / IDE i18n / 等 5 大类。**68+ capabilities (+3 activity timeline), 12 proactive triggers (6 cron + 5 activity 快路径 + 1 activity_judge 慢路径), 1700+ tests / 0 regressions, 11 abstractions** — CapabilityRegistry / ProactiveTrigger ABC / bidirectional MCP / per-tool MCP toggle / SAFE path util / mpv-IPC wrapper / ActivityWatcher / CapabilityRow accordion / ProviderGroupRow / ActivityJudge slow-path / **ActivityTimeline session writer**。Next up: chunk 8b 完整屏幕感知。
>
