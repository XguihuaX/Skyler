...
> **Status (May 2026)**: v3 ✅ complete + v3.5 chunk 5 ✅ shipped (per-character `background_path` layer + launch `intro.mp4` splash video with silent-skip on missing file). Full v3-A through v3-H chunk 1 + v3.5 chunk 5; 30+ capabilities, 6 proactive triggers, 530+ tests / 0 regressions, 3 abstractions—CapabilityRegistry / ProactiveTrigger ABC / bidirectional MCP—powering everything. Next up: chunk 6 媒体接入收尾（B 站 + 网易云自解码 + 小红书 URL 解析）/ chunk 7 skill 集成 demo（docx capability + Notion MCP server registry）/ chunk 8 v4 屏幕感知（VLM 抽象 + Tauri 截图 + 隐私黑名单）。
>
...
| v3.5 chunk 7: skill integration demo (docx capability stance A + Notion MCP server stance B) | 📋 planned |
