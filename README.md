...
> **Status (May 2026)**: v3 ✅ complete + v3.5 chunk 5 ✅ + chunk 7 ✅ shipped. chunk 7 兑现「个人乐高底盘」承诺——two skill integration stances live: 姿态 A (本地 capability, demo: docx via python-docx + SAFE sandbox + path-traversal defense) and 姿态 B (one-click MCP server enable, demo: official `@notionhq/notion-mcp-server` with UI-driven DB-stored credentials via new `mcp_credentials` / `mcp_client_state` tables). 33+ capabilities, 6 proactive triggers, 595+ tests / 0 regressions, 4 abstractions—CapabilityRegistry / ProactiveTrigger ABC / bidirectional MCP / SAFE path util. Next up: chunk 6 媒体接入收尾（B 站 + 网易云自解码 + 小红书 URL 解析）/ chunk 8 v4 屏幕感知（VLM 抽象 + Tauri 截图 + 隐私黑名单）。
>
...
| v3.5 chunk 6: media wrap-up (Bilibili capabilities + NCM mpv-based autoplay redo + Xiaohongshu passive URL parser) | 📋 planned |
...
| v3.5 chunk 8: v4 screen awareness (VLM abstraction + Tauri screenshot + pixel-diff prefilter + privacy blocklist) | 📋 planned |
