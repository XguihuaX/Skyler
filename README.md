...
> **Status (May 2026)**: v3 ✅ complete (full v3-A through v3-H chunk 1; 30+ capabilities, 6 proactive triggers, 500+ tests / 0 regressions, 3 abstractions—CapabilityRegistry / ProactiveTrigger ABC / bidirectional MCP—powering everything). Next up: chunk 5 视觉跃迁包（角色背景层 + 启动 splash video）/ chunk 6 媒体接入收尾（B 站 + 网易云自解码 + 小红书 URL 解析）/ chunk 7 skill 集成 demo（docx capability + Notion MCP server registry）/ chunk 8 v4 屏幕感知（VLM 抽象 + Tauri 截图 + 隐私黑名单）。
>
...
| v3.5 chunk 5: visual upgrade (per-character background layer + Tauri splash video) | 📋 planned |
