...
> **Status (May 2026)**: v2.7 backend & UI complete · v3-A/B/C/D + v3-E1/E2 + v3-G' + **v3-G chunks 0 + 1 + 1.5 + 1.6** done (~95% of v3) · Live2D + seven-voice CosyVoice + Capability Registry + bidirectional MCP + dual-source calendar (Apple Calendar via macOS EventKit *and* Google Calendar via OAuth, switchable in one yaml line; Apple is default for China users — zero VPN required) + morning briefing + create/delete event support · next up: v3-G chunk 2 (briefing intelligence + proactive audio) + chunk 2.5 (NL event entry).
>
