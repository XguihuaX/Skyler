...
> **Status (May 2026)**: v3 ✅ complete + v3.5 chunks 5 / 6 (a/b/c) / 7 / 8a / 9 / 10 / 11 ✅ + UX-001 ✅ + hotfix-6 ✅ + UX-002 ✅ + hotfix-7 ✅ + hotfix-8 ✅ shipped. Memory 升级到**三层结构**(chunk 9 遗忘曲线 + chunk 10 server-side MemoryExtractor worker + chunk 11 structured `profile_data`)。chunk 8a 简化屏幕感知(active app + browser URL + 公开页面正文 + smart proactive trigger,pyobjc NSWorkspace + osascript,全本地零云端)。UX-001 MCP per-tool accordion + 情绪 UI 避开 TopBar；hotfix-6 chunk 8a 切 VSCode 无主动消息根因修 + 6+ 条 INFO 级 trigger-chain log；UX-002 SettingsPanel→CapabilityPanel 67 cap 全面 accordion 化 + 删冗余 MCP banner + calendar Google OAuth 归位;hotfix-7 proactive trigger state_update tag strip 漏防修 + WS 5 道 strip 兜底契约 + TTS toggle "undefined" 真因修(Tauri Rust Err normalize);**hotfix-8 config.yaml duplicate block 合并(chunk 8a docs commit 非幂等 restore 之锅) + _IDE_APPS 中文 macOS i18n 加 `'终端'` 等 9 个 alias + commit hotfix-5 Info.plist + .gitignore config backup pattern**。**65+ capabilities, 11 proactive triggers (6 cron + 5 activity-based), 1200+ tests / 0 regressions, 8 abstractions** — CapabilityRegistry / ProactiveTrigger ABC / bidirectional MCP / per-tool MCP toggle / SAFE path util / mpv-IPC wrapper / ActivityWatcher / CapabilityRow accordion. Next up: chunk 8b 完整屏幕感知(截屏 + OCR + VLM 抽象 + 浏览器扩展)。
>
