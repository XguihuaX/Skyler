...
> **Status (May 2026)**: v3 ✅ complete + v3.5 chunk 5 ✅ + chunk 7 ✅ + chunk 6a ✅ shipped. chunk 6a delivers 11 Bilibili capabilities including the killer use case `bilibili.get_subtitles` + LLM → video summarization ("帮我总结这个 B 站视频" works end-to-end). 6 no-cookie + 5 cookie capabilities (cookie via `.env` `BILIBILI_SESSDATA`, same pattern as netease `MUSIC_U`). 44+ capabilities, 6 proactive triggers, 700+ tests / 0 regressions, 4 abstractions—CapabilityRegistry / ProactiveTrigger ABC / bidirectional MCP / SAFE path util. Next up: chunk 6b 网易云自解码 + chunk 6c 小红书 URL 解析 / chunk 8 v4 屏幕感知（VLM 抽象 + Tauri 截图 + 隐私黑名单）。
>
...
| v3.5 chunk 6b/6c: NCM mpv-based autoplay redo + Xiaohongshu passive URL parser | 📋 planned |
