...
> **Status (May 2026)**: v3 ✅ complete + v3.5 chunks 5 / 6 (a/b/c) / 7 / 8a / **8a-ext** / **8a-ext V2** / 9 / 10 / 11 ✅ + UX-001/UX-002/UX-003 ✅ + hotfix-3 ~ 10 ✅ shipped. Memory 三层结构(chunk 9 + chunk 10 worker + chunk 11 profile_data)。chunk 8a 简化屏幕感知 + **chunk 8a-ext 智能陪伴 judge 慢路径** —— 在快路径(IDE/音乐/技术文档)未命中时,用 qwen-turbo 判断用户停 5+ min 的 app/URL 是否值得主动 chime in,yes 走 ChatAgent 主对话,no 静默。三重门(min_stay 5 min + judge throttle 10 min + fire throttle 30 min)+ 共享 daily_cap (5/天) 防滥用。topic_hint 作 anchor 注入主 LLM 让 Momo 知道往哪个方向搭话。**chunk 8a-ext V2 加 idle 第 4 道闸** —— ``ioreg -c IOHIDSystem`` 抽 ``HIDIdleTime`` 纳秒转秒,键鼠静止 > 300s 视为离开电脑,judge 直接 skip 不烧 LLM 不"自言自语"。零新 pip 包(复用 chunk 8a subprocess 范式)+ 跨平台 graceful(非 macOS 返 None 保持 V1 行为)。SettingsPanel 二级 toggle + 三级 idle 阈值输入(0..3600s,0 = 关闸),默认 ON 可关。UX-001/UX-002/UX-003 SettingsPanel 三层 accordion 全栈完成 + 情绪 UI 左上角。hotfix-3..hotfix-8 治本 strip / TTS / config duplicate / IDE i18n / 等 5 大类。**65+ capabilities, 12 proactive triggers (6 cron + 5 activity 快路径 + 1 activity_judge 慢路径), 1500+ tests / 0 regressions, 10 abstractions** — CapabilityRegistry / ProactiveTrigger ABC / bidirectional MCP / per-tool MCP toggle / SAFE path util / mpv-IPC wrapper / ActivityWatcher / CapabilityRow accordion / ProviderGroupRow / ActivityJudge slow-path。Next up: chunk 8b 完整屏幕感知。
>
