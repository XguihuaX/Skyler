...
> **Status (May 2026)**: v3 ✅ complete + v3.5 chunks 5 / 6 (a/b/c) / 7 / 8a / 9 / 10 / 11 ✅ + **UX-001/UX-002/UX-003** ✅ + hotfix-3 ~ 8 ✅ shipped. Memory 三层结构(chunk 9 遗忘曲线 + chunk 10 worker + chunk 11 profile_data)。chunk 8a 简化屏幕感知(active app + browser URL + smart proactive trigger)。**UX-001/UX-002/UX-003 SettingsPanel 三层 accordion 全栈完成** —— category(9 个,默认折叠)→ provider(多 provider category 内 calendar/mcp_external/media 自动二层分组)→ capability(单 cap 行内 description / 谁能调 / 触发等展开)。情绪 UI 从右上角挪左上角避开 ``[历史]`` 按钮。hotfix-3..hotfix-8 治本 state_update tag strip 漏防 / TTS toggle undefined / config.yaml duplicate block / IDE i18n / 等 5 大类问题。**65+ capabilities, 11 proactive triggers (6 cron + 5 activity-based), 1200+ tests / 0 regressions, 9 abstractions** — CapabilityRegistry / ProactiveTrigger ABC / bidirectional MCP / per-tool MCP toggle / SAFE path util / mpv-IPC wrapper / ActivityWatcher / CapabilityRow accordion / ProviderGroupRow(三层 fold)。Next up: chunk 8b 完整屏幕感知。
>
