...
- **文档债** —— 中英 README 结构不对称(英文有独立 `## Known Problems` 编号段,中文并入本「已知限制」列表);DESIGN.md 待大整合(双层保留 / 旧"当前"标签 / chunk 章收并,立项留待表层重构 pass)。后续统一,本批不做。
...
- **v4.0.0 收口(进行中)**:长期记忆链路审计 ✅(修复链已 ship,代码核验;待真机回归)→ TTS 每用户日字数上限 + 主对话节流 → Stage3 Tauri 打包 / dmg / dogfood / tag
...
剩余 v4.0.0 收口项:长期记忆链路审计 ✅(修复链已 ship 且代码核验;待真机回归)→ TTS/对话成本闸 → Stage3 打包发布。完整实施日志(每个 chunk、hotfix、UX 迭代)在 [ROADMAP.md](ROADMAP.md) —— 那是真实的历史。
