...
> **最新**:v4-beta (2026 年 5 月) —— Persona Engineering 五层框架 + 记忆/对话隔离根治 + conversation 锚定绑定语义 + 对话 UI 统一。当前主推 **Mai 单角色纯中文陪伴**(其他角色 v4.1 上线)。完整路线见 [ROADMAP](ROADMAP.md)。
>
...
**当前主推 Mai(`cid=1`,借 Momo 壳 + Hiyori 模型,内核是樱岛麻衣 persona)单角色、纯中文陪伴。**

本 session 已根治并真机验证通过:

- **语言回退纯中文** —— 日语原声链路(`<ja>` tag 交替合成)折腾多版稳定性不达标,v4.0.0 决定 Mai 回退纯中文(人格完全不动,只换语音链路)。日语原声留 v4.1 用**后处理翻译架构**(LLM 出纯中文 → TTS 前翻译 → 合成)一次做对。ja 链代码全部保留休眠。
...
- **token 成本治理** —— 短期记忆硬性 cap 最近 30 turn;tool result 注入截断到 4000 字符。多 round 工具调用不再把单次输入推到几万 token。

**已知限制(诚实列,这就是路线图):**

- 其他角色(八重神子等 `cid=2/3/...`)目前是**空骨架**,没有完整 persona,切过去人格空洞。v4.1 (F1) 逐个补真 persona。
- **长期记忆链路 v4.0.0 审计中** —— 收口审计发现默认用户 `memory` 表写入为 0 行,长期事实记忆的提炼链路是否真正生效正在排查。这意味着当前角色对超过 30 turn 之前的内容可能"失忆"。这是 v4.0.0 必修项,不是 v4.1。下面"记忆 & 人格"一节如实标注了这个状态。
...
- **TTS** —— CosyVoice(DashScope,默认)→ Edge-TTS 后备;每角色独立 `voice_model` 配置;SoVITS 计划中
  - **v4-beta**:主推角色 Mai 已回退**纯中文**(中文音色 `longyumi_v3`)。日语原声链路(`<ja>` 交替合成)保留代码但休眠,v4.1 用后处理翻译架构(LLM 出纯中文 → TTS 前 qwen-turbo 翻译 → CosyVoice)重做,把"LLM 自己实时交替标 ja"这个不确定性彻底移出链路。
...
> **v4-beta 审计声明**:下面第 2 层(长期事实记忆)的入库链路在 v4-beta 收口审计中发现对默认用户写入为 **0 行**。提炼 worker 是否真正触发、写入是否成功,是 v4.0.0 必查项。在 audit 结论出来前,长期记忆**实际可能未生效**,角色对 30 turn 之前的内容会失忆。文字描述保留设计意图,但请以此声明为当前真实状态。
...
- **第 2 层 长期事实记忆** —— `memory` 表,设计入库主路径为 server-side worker(`MemoryExtractor`,每 5 min batch 提取 + 10 道 quality filter + entry_type 四分类 + extraction_source 四态来源标签);`save_memory` tool 为"用户明确说要记"的显式入口;检索按遗忘曲线 `score = relevance * (1+log(1+ac)) / (1+age*decay)` + threshold gate。**⚠️ 见上方审计声明:该链路 v4.0.0 审计中,当前实际写入为 0。**
...
### 🔔 主动陪伴
- **Trigger pack** —— wake_call / lunch_call / dinner_call / bedtime_chat / long_idle / morning_briefing —— 角色在该出现的时候才出现,不是每个 polling 都说话
...
用户输入(语音 / 文字)
  ...
  → ChatAgent  context 装配 + LiteLLM tool calling
                ├─ short_term:(用户,角色,conversation) 三级隔离,cap 30 turn
...
- **TTS**:CosyVoice(DashScope)默认 / Edge-TTS 后备 / SoVITS 计划中(v4-beta Mai = 纯中文)
...
- v4-beta **暂不**开放多角色完整体验 —— 当前主推 Mai 单角色纯中文。其他角色 persona + 日语原声 + 长期记忆链路均为 v4.0.0/v4.1 收口项,诚实列在上面"v4-beta 当前状态"和路线图里。
...
- **v4.0.0 收口(进行中)**:长期记忆链路审计 → TTS 每用户日字数上限 + 主对话节流 → Stage3 Tauri 打包 / dmg / dogfood / tag
- **v4.1**:ja 后处理翻译重做(F0)/ 七套角色真 persona(F1)/ 长期记忆归属分级(F8)/「记忆架构 v2」(一角色一永久对话流 + RAG)/ 切角色对话联动收尾(F2)/ LLM 性能 / CosyVoice 弱网超时 / 测试债清理
...
**v4-beta 收口阶段(2026 年 5 月)。** Persona Engineering 五层框架 + 记忆/对话三级隔离 + conversation 锚定绑定语义 + 对话 UI 统一已 ship 并真机验证。当前主推 Mai 单角色纯中文陪伴。

剩余 v4.0.0 收口项:长期记忆链路审计(关键,见上方记忆审计声明)→ TTS/对话成本闸 → Stage3 打包发布。完整实施日志(每个 chunk、hotfix、UX 迭代)在 [ROADMAP.md](ROADMAP.md) —— 那是真实的历史。
