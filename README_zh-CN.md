...
[观看 Skyler Demo v0.1](https://github.com/XguihuaX/Skyler/releases/tag/demo-v0.1)
...
- Tier-2 指 Persona 扩展字段，例如 `taboo_topics`、`lore` 和
  `capability_overrides`。
- `lore` 指长期背景设定，不是 LoRA。
- LoRA 是模型微调技术，不属于 Persona 字段。
- AX / UI Tree 是按需结构化读屏，不是持续视觉感知。
- DailyAgent 目前是 Stage 1，不是完整 FSM。
- motion / 动作库 / ComfyUI / Anime 资产生成仍是后续方向。
