...
|---|---|---|---|
| 📋 | **v4.1+ Mai voice fidelity 升级 · 远程 GPU fine-tuning + zero-shot hybrid** | INV-9 中插 sweep 实证 zero-shot from 7s reference 有硬天花板(per audit `b34ad70` Lesson INV-9 #8 stochastic 验证 + 43 WAV outputs 听感对比 Part 1/Part 2/repro);本轮 Phase 2 ship 接受 stochastic 作 Fish s2-pro 固有特性。**v4.1+ fidelity 升级路径**:用 `tts/fish/参考音频/mai/` 完整 5min Mai 素材 + 远程 GPU fine-tune Fish 模型 / 或自训 SoVITS / 或 cosyvoice-v3.5-plus 复刻进阶;新模型 voice 与现 zero-shot 路径并存,UI CharacterPanel 提供切换。**触发**:v4.1+ 真要做产品级 Mai 音色时启动 | 立项 v4.1+,reference `b34ad70` audit + Lesson INV-9 #8 + `mai5min/` 整 5min Mai 素材 |
