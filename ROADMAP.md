...
## MCP 能力层 batch(2026-06-15~16 · 分支 `gsv-thin-and-global-server` · **HOLD 待真机验收 + 4 logical commit + push**)

> 状态分层(2026-06-16):
> - **已真机端到端验**(1/N):xhs 换包 rednote-mcp · 扫码 → cookie 落 4.3KB · enable → list_tools 拉到 4 只读 tool · search_notes 实搜出小红书结果(PM 真机过)
> - **已建 · 单测/cold-check 过 · HOLD 待 PM 真机全链路 + 4 logical commit + push**:holder-task / SSE Amap / 配置分文件 / per-tool confirm gate / browser_login 流 / batch2 自校验+45s+queue+deny_all_pending
> - **本批不产 commit**(本地):`mcp.config.yaml`(gitignored 真名单 · 永不入 stage)+ `config.yaml`(workaround TD-G)
...
| 主题 | 状态 | Notes |
|---|---|---|
| 🟡 **② holder-task 模型(解 anyio 跨-task cancel-scope)** | 代码完 · cold check 过 · **未 commit · HOLD** | `backend/mcp/client.py` · `_ClientHandle` 加 4 字段(`stop_event/ready_event/holder_task/connect_error` L92-94)· 新 `_holder_task` L142(async with AsyncExitStack 的 enter + `ready_event.set` + `await stop_event.wait()` + 自然退栈 **全在同一 task**)· `_start_holder` L400 / `_stop_holder` L427 外部接口(set stop_event + await)· 解原 `stack.aclose` 跨 task raise `Attempted to exit cancel scope in a different task`。HOLD 等真机:enable/disable 反复 N 轮无残留 task / 关 backend 时 holder 干净退 / `mcp-holder-<name>` task 名能 grep 到 |
| 🟡 **③ SSE transport(Amap)** | 代码完 · 单 entry 配置就位 · **未 commit · HOLD** | `client.py:177-214` 加 `elif transport_kind == "sse"` 分支 · `mcp.client.sse.sse_client(url, headers)` · `url` 内 `${VAR}` 展开走 `_expand_str(conf)` · cred-aware:DB credentials 临时 patch `os.environ` 期内,SSE 走 URL 含 key 拿 token(不像 stdio 通过子进程 env 注入)· `mcp.config.yaml` amap entry:`transport: sse` + `url: https://mcp.amap.com/sse?key=${AMAP_MAPS_API_KEY}` + `env_required: [AMAP_MAPS_API_KEY]` · 兜底留 `amap-stdio` 同 key 同 server。HOLD 等真机:PM 填 key → enable amap → search_location 真返地理结果 |
| 🟡 **④ 配置分文件(mcp.config.yaml ↔ example)** | 代码完 · 16 entries 已搬 · loader merge OK · **example 待 commit · HOLD** | 新增 `mcp.config.yaml`(本地 · gitignored 入 `.gitignore`)+ `mcp.config.example.yaml`(入库 · 模板)· `backend/config/__init__.py:40 load_config_yaml()` 加 mcp_path 检测 + merge(L59-78):mcp.config.yaml > example > none · 整段覆盖 `mcp_clients` + `mcp_server` 两 key 不深合并 · `backend/routes/mcp_api.py` `_MCP_CONFIG_PATH` POST/DELETE 写新文件 · `config.yaml` 删 mcp_clients / mcp_server 段(workaround TD-G 长期 modified · 真删但不 commit) · 16 entries 含 8 原有 + 7 新(amap / amap-stdio / akshare / email / trip12306 / xmind / xhs / rss-reader)+ 1 个 inv 测试用 filesystem-test。HOLD 等真机:启动日志 `mcp.config.yaml` 命中 + status 接口 16 行 |
| 🟡 **⑤ per-tool confirm gate(dangerous_tools)** | 代码完 · 全链路单测过 · **未 commit · HOLD** | migration `inv_mcp_tool_confirmation.py`(ALTER TABLE mcp_tool_state ADD COLUMN require_confirmation INTEGER NOT NULL DEFAULT 0 · 幂等 PRAGMA table_info 检查)· `backend/mcp/confirm_gate.py` 新文件(`ToolConfirmationRejected` 非 CancelledError L43 · `register_push_callback` L75 · `request_confirmation` 120s timeout L111 · `resolve_confirmation` L200 · `deny_all_pending` L218)· `backend/mcp/tool_state.py` 加 `is_confirmation_required` L37 / `seed_require_confirmation` L81(幂等 INSERT OR IGNORE) / `set_require_confirmation` L110 · `backend/routes/ws.py` 早路由 `mcp_tool_confirm_response` + finally 链 `deny_all_pending` + `register_push_callback(None)` · 前端 `MCPConfirmModal.tsx` + `store.mcpConfirmQueue` 队列化 + `useWebSocket` enqueue/shift/clear · `mcp.config.yaml` github 12 条 / email 3 条 / xhs `[login]`(防 LLM 自调弹浏览器)。HOLD 等真机:LLM 触发 dangerous tool → modal 弹 → accept/reject/timeout 三态都过 + WS 断线时 pending confirm 自动 deny |
| 🟡 **batch2 [自校验 + 45s 超时 + queue + deny_all_pending]** | 代码完 · **未 commit · HOLD** | `client.py` ENTER list_tools 后 reconcile `dangerous_tools` 配置 ↔ 真 tool list:`stale_dangerous`(配置写了真没的 tool 名 · WARN)+ `possibly_naked`(真 list 含 push/delete/send 等写操作但未列 · WARN)· `_CALL_TOOL_TIMEOUT_SECONDS = 45.0` L62 包 `asyncio.wait_for(session.call_tool)` · 前端 store `mcpConfirmRequest: T | null` → `mcpConfirmQueue: T[]` · `Modal` 渲队首 + "队列 +N" 角标 · WS `onclose` 前端 `clearMcpConfirmQueue`,后端 `deny_all_pending` |
| 🟡 **batch2 [browser_login] · `auth: browser_login` 链路** | 代码完 · **xhs 已端到端真机过(✅)** · 其它 browser_login 类未来加 entry 可复用 | `backend/mcp/browser_login.py` 新文件(`_LoginTask` 状态机 4 态 · `start_login` L89 async subprocess + watcher · `_watch_login_proc` L165 10min 扫码窗 · `cookie_ready` L75 · `get_login_status` L209 · `is_browser_login_entry` L236)· `mcp_api.py` 加 `POST/GET /api/mcp/clients/{name}/login`(L130/L154)· `client.py:168` `_holder_task` ENTER 前 `cookie_ready` 检查 → raise FileNotFoundError 直接 fail 不开浏览器 · `client.py:838-839` `list_status` 出口加 `auth` + `login` 字段 · `mcp_api.py:71 MCPLoginStatusItem`(Literal 4 态) + L78 `MCPClientStatusItem` 加 `auth` + `login`(**Item 1 修:原 schema 没声明 → FastAPI response_model 静默剥字段 → 前端 client.auth=undefined → 按钮不出**)· 前端 `lib/mcp_clients.ts` 加 `MCPLoginStatus` + `startMCPBrowserLogin` + `getMCPLoginStatus` · `ExtensionsSection.tsx` 按 `isBrowserLogin` 分支按钮(登录/重新登录/扫码中 三态)+ 黄字 / 灰字状态 + 1.5s 轮询 GET 至 cookie_ready · `mcp.config.yaml` xhs entry 切 rednote-mcp(`/Users/.../nvm/.../bin/rednote-mcp --stdio` · `login_args: [init]` · `cookie_path: ${HOME}/.mcp/rednote/cookies.json` 跟内部硬编码对齐 · `dangerous_tools: [login]`) |
| 🟢 **xhs rednote-mcp 真机** | **shipped + 真机 ✓(2026-06-16)** | rednote-mcp v1 全只读 4 tool · `~/.mcp/rednote/cookies.json` 4.3KB 落地 · backend 拉 stdio 启动 OK · search_notes 实搜出小红书结果 · 唯一已真机端到端跑过的 MCP entry(其它 ② 到 ⑤ HOLD 待 PM 真机) |
| 📋 **example.yaml committable + 4 logical commit 切片** | 待 PM 真机过完整链路后 | 提交切片(`git add -p` 隔离 · 不带 GSV / `config.yaml` / `mcp.config.yaml`):① Item 1 schema 修 `backend/routes/mcp_api.py`(`MCPLoginStatusItem` + auth/login 字段进 `MCPClientStatusItem` + 同步注释)② ② holder-task + 必要 client.py 改 ③ ③ SSE + ④ 拆文件(`backend/config/__init__.py` + `backend/routes/mcp_api.py::_MCP_CONFIG_PATH` + `.gitignore` + `mcp.config.example.yaml` + `backend/mcp/client.py` SSE 分支)④ ⑤ confirm gate(migration + confirm_gate.py + tool_state.py + ws.py + 前端 store/hooks/Modal)⑤ batch2(client.py 自校验+45s + 前端 queue + ws.py 链尾)⑥ browser_login(browser_login.py + client.py 入口/list_status + mcp_api.py routes + lib/mcp_clients.ts + ExtensionsSection.tsx + example.yaml 范式 6)|

> 详 DESIGN_LITE §7.7 · 关联 Tech Debt 表"MCP 能力层衍生"(下方新加)· 关联 P3 backlog "MCP marketplace"(决策 #1 旧条 · 不替代 · 是预设清单的更上层)。

---

## GSV 本地 TTS 迁移(2026-06-11~14 · 分支 `gsv-thin-and-global-server` · **IN PROGRESS · 未 commit**)

| 主题 | 状态 | Notes |
|---|---|---|
| 🟡 **全局 server_url + DB 管理 tts_models 表** | 代码完 · 真机部分验证 · **未 commit** | 分支 `gsv-thin-and-global-server` 工作树堆积:`backend/tts/gsv_settings.py`(全局 server_url module cache · sync sqlite 读 ai_providers + setter + cleanup `_MODEL_LOADED_KEYS`)+ `backend/tts/tts_models_cache.py`(sync cache + DB tts_models 表 + 失败 fallback tts_models.json)+ migration `inv_tts_models_table_and_seed.py`(CREATE TABLE + mai_v4 seed)+ `inv_voice_model_thin_strip_gsv.py`(strip cid=1 voice_model spread)+ `backend/routes/tts_models_api.py`(CRUD)+ tts_api 加 server_url GET/POST + emotion_coverage + refs · 前端 `AddGsvModelModal.tsx` + `lib/tts_models.ts` + GsvTTSCard 重做(卡顶 server_url + Save + ping · model 列表 add/edit/delete · 折叠情绪覆盖)· VoicePicker gsv 分支 thin reference · 三 tier `DB voice_model > 全局 > _DEFAULT` |
| 🟡 **GSV httpx 客户端绕代理 + mai_v4 wav_remote_dir 改本地** | 代码完 · 本机已 synth 真音频 ✓ · **未 commit** | `backend/tts/gsv.py` 3 处 httpx + `routes/tts_api.py::gsv_ping` 全加 `trust_env=False`(绕过 shell `HTTP(S)_PROXY` · 不依赖 NO_PROXY)· migration 加幂等 UPDATE:`wav_remote_dir` 仍等于旧公网 `/workspace/GSVI/mai_emotion_bank/` → 迁本地 host `D:/GPT-sovits/GPT-SoVITS-v2pro-20250604-nvidia50/reference_audio/mai_v4/`(只在等于旧值时改 · 不覆盖用户手动设的)· seed default 同步改 · **本机已验**:重启后 init 日志 `remote_bank=D:/GPT-sovits/.../mai_v4/` · 连上 GSV 局域网 + 载权重 + 出真实音频(非 stub) |
| 🔴 **未决:中文输出音色"不对"** | 调查中 · 下一轮另开对话 | 疑似跨语种(mai_v4 是日语训练音 · 被 voice_model.tts_language=zh 逼着读中文 → 音色飘)· 跟 layer_a.j2 `voice_provider=='gsv' && voice_model_name=='mai_v4'` 段 + 渲染期 voice_model.tts_language 路由有交互 · **不算 done**(连接通 + synth 出真音不等于音色对) |

> **此段不进 DESIGN_LITE / README 当既定架构**:整批未 commit · 真机仅本机走通局域网连接 + 单角色 ja 路径 · 中文音色飘未解 · 推到下一轮对话继续。新接手优先看分支工作树 status + 这条 IN PROGRESS 状态。
...
| Brick | 内容 | 依赖 | 状态 |
|---|---|---|---|
| #1 | **pixiCubism4 表演层 = 活物基线**(idle sway BodyAngleY/Z ±1.5° + head-turn focus GAIN ±15° + 眨眼 + 呼吸 + 物理/姿态 SDK 自动 idle) | — | ✅ shipped(2026-06-14 commit `c14065b` · 详 §7.6) |
| #2 | `<emotion>` → **Live2D expression 切换**(最小可见 · 单角色试 · 单仓库;复用已 parse 的 emotion 信号) | sanitize chain 已提 `<emotion>`(§5.6 / 数据流 L121)| 📋 backlog · 起步砖 · 0.5d |
| #3 | **motion bank** + 选择策略(tag schema · cooldown · weighted random · 防过密) | brick #2 | 📋 backlog · 1-2d |
