...
| 📋 | extractor 5-min 频率 audit | `backend/memory/extractor.py:445 run_loop()` 默认 `get_extractor_interval_seconds()` = 300s,每用户每 tick 跑 LLM-driven dedup + fact extraction。频率是否合理 / 能否拉长省 LLM 成本未审计。子轨 A/B 期间 PM 提出怀疑,挂 backlog 单独刀 audit | 立项 backlog |
