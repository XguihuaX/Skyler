...
| Live2D | AI 角色导演 — VTuber 是被人操的 → Skyler 要当操偶人 · LLM 在 curated motion/expression repertoire 上做导演 · 输入 说话/听/思考 state + `<emotion>` 信号 · §7.6 表演层(`c14065b`)是第一块砖 · 复用 `<emotion>` 驱动表情切换起步 | 长期方向 |
