...
| 📋 | chunk 15 / UX-006 — Audio streaming | Sentence-by-sentence TTS 治 23s tool-call 沉默(memory #5 必做) | ~1.5 days |
...
**为什么 chunk 15 必做**:UX-004 v1 实测 qwen 3.6 默认 silent tool_use(LLM 内部决定 tool,tool 返回后一次性输出"过渡语 + 最终回复"合并 message)—— prompt 引导让 100% 输出过渡语文字,但无法控制 token stream 顺序,体感仍是 23 秒沉默→一起到。彻底治本要 backend message 拆分 + audio pipeline sentence-by-sentence streaming + pre-tool/post-tool 两段 TTS。详见 [DESIGN §十五之V](DESIGN.md)。
