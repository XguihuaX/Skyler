...
> **状态(2026-05-13)**:v4-alpha shipped。当前重点是把可塑性做到下一个贡献者也能顺手用。
...
目标不是堆 feature,是把现有可塑性的边缘磨光 —— 让外部贡献者写一个 skill、换一个 Live2D 模型、调一个 LLM provider 不需要花一天读源码。

| Status | Item | Goal | ETA |
|---|---|---|---|
| ✅ | chunk 14 — Activity Timeline | 跟 chat_history 平行的每日活动 timeline + LLM context 注入 | shipped 2026-05-13 |
| ✅ | UX-004 v1 — Tool-call transition | 文字过渡语 + frontend loading pill(audio streaming 留 v2) | shipped 2026-05-13 |
| ✅ | UX-005 — Capability 单一归属 | media/music/social/clipboard 分类清理 + 剪贴板 accordion | shipped 2026-05-13 |
| ✅ | UX-007 — Momo bubble fade | 1 min 渐进淡化 + ChatHistoryDrawer 半透明 overlay | shipped 2026-05-13 |
| 🚧 | chunk 12 — Persona depth | Momo / 八重 完整 persona + 角色化过渡语 examples | ~1 day |
| ✅ | chunk 15 / UX-006 — Audio streaming | 经实测体感无沉默,关闭(closed by audit, not implemented)。详 [docs/chunk-15-*](docs/) | closed 2026-05-14 |
| 📋 | **Stage 2 — 纯前端管理三类资源** | 用户不动 yaml / 不改 .py,UI 直接装(详 [docs/stage-2-starting-context.md](docs/stage-2-starting-context.md))| ~5-7 days |
| 📋 | ↳ Stage 2.1 — MCP 表单加新 server | `POST /api/mcp/clients` + 前端 ExtensionsSection 表单 + 删除 | ~2-3 days |
| 📋 | ↳ Stage 2.2 — Live2D 拖入 .moc3 package | zip 上传 → safe_path 解压 → scanner 自动识别 + 自动 motionMap 默认值 | ~3-4 days |
| 📋 | ↳ Stage 2 — 改写 skill 接入 guide | 现有 [skills-extension-guide.md](skills-extension-guide.md) 加 § "UI 直接装" fast path,代码路径降为 advanced | ~0.5 day |
| 📋 | chunk 13 — Test isolation | Pre-release test debt:fixture 隔离 + 全套 pytest 跑通 | ~2 days |
| 📋 | Skill docs + examples | 30-min 上手:三个完整示例(同步 / 异步 / 多 Consumer) | ~1 day |
| 📋 | Live2D swap guide | Step-by-step:从拿到 model 到 in-app 可用 | ~1 day |
| 📋 | Plugin registry seedling | `docs/plugins/` markdown index 起步,长期演化 web index | ongoing |

**chunk 15 关闭说明**:UX-004 v1 实测 qwen 3.6 在某些环境下体感 23s 沉默(memory #5)。经 4 阶段 audit(starting context → feasibility → backend profile → frontend profile)+ 关 VPN 真机实测,backend producer/consumer + chunk 6b TTS pipeline 已实现 sentence-by-sentence streaming,过渡语 + 最终回复语音流畅。"23s 沉默"推测为 VPN + 第一次冷启 tool 叠加偶发现象,非架构问题。工时实际:0 实施(audit ~3h CC + 用户 30min profile)。详 `docs/chunk-15-b1-profile.md` §5。
...
| 数据架构 | Characters 双源(`characters.yaml` + DB)—— 方案 C 删 yaml DB 单源 | v4 后期 |
...
| 角色 | Momo (id=1) 当前 Yae Miko 临时占位,chunk 12 恢复原 ChatAgent 人设 | chunk 12 同期 |
