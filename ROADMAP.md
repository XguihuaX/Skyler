...
|---|---|---|---|
| 🚧 | **文档重写** | README / README_zh-CN / ROADMAP / DESIGN_LITE / DESIGN_patch 对齐 v4-beta 现状(handoff 给新对话用)| 进行中 |
| 📋 | **长期记忆链路 audit(critical)** | 收口审计发现默认用户 `memory` 表 0 行;提炼 worker 是否真触发/写入是否成功,只读 audit → 据结论修。陪伴命门,不是 v4.1 | ~待 audit |
...
| 📋 | **F8 — 长期记忆归属分级** | fact/profile → user_shared;event/关系型 → character_private(按 character_id);short_term 已 conv 隔离。**前提"long-term 在工作"已被 v4.0.0 audit 推翻(0 行),先解决"有没有"(v4.0.0)再"分级"(v4.1)** | 依赖 v4.0.0 audit 结论 |
...
| 记忆 | **长期记忆链路 audit(v4.0.0 critical)** —— 默认用户 `memory` 表 0 行,提炼 worker 是否真触发待查;陪伴命门 | v4.0.0 收口项 |
