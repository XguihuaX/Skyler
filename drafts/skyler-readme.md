...
- **本地语音落地**(2026-06-17 真机验收,06-11~17 窗口)—— 自训音色 GSV `mai_v4` + 16 情绪,从云端迁到局域网本地机,推理不出公网。
- **桌面感知(只读)** —— 角色能读当前前台窗口的 UI 树(macOS 无障碍 AX)并据实回答;读屏只读,写动作能力已建但确认门未验,建议先手动关。
- **MCP 能力层 + 管理** —— 接任意 MCP server + 服务器搜索 / 别名。
- **深度思考 / 联网双开关** —— 聊天框 + 设置双入口,默认快(关思考),想深再开。
- **Live2D 取景 + 表演层** —— 待机微动 / 转头看你 / 眨眼呼吸 / 口型 + 构图可调。
- **玻璃外观自定义 + 角色详情中心** —— 陪伴态浮窗按壁纸自调对比度;立绘馆里看人格 / 心情 / 状态并就地编辑 persona。
...
能力注册表统一管原生能力和 MCP 工具;写 / 变更类工具进 `dangerous_tools`、调用前二次确认;桌面控制**读写分离** —— 读 UI 树安全、写动作必须过确认门且默认关。当一个 agent 能调外部工具、还能读屏 / 控桌面时,这层是必需品,不是装饰。
