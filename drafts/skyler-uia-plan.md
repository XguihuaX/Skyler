# Skyler UIA(桌面感知/控制)规划 — 最终目标 + MVP 现状

> 内部规划文档。`✅` = 已落地/已验证,`🔨` = 进行中,`📋` = 规划(未建)。
> 当前代码现状由 CC 真机调查确认;规划项均明确标注,不与现状混写。
> 更新:2026-06-21
...
- 🔨 **方案 B 已 greenlit,CC 正在 build**(`read_current_screen` capability + prompt)→ 报两个 diff → HOLD → 我审 → commit → 真机验。
- ✅ 已确认的底子(CC 真机调查):macos-use **当前就活的**(DB override enabled,9/9 tool 注册进 ChatAgent);ChatAgent 是多步 loop;`refresh_traversal` 是唯一纯读(需 PID);`open_application_and_traverse` 等 8 个是写、在 dangerous_tools。
