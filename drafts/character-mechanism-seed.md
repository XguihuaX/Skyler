# 角色机制 · 设计种子(parked · 后续展开为 docs/design/character-mechanism.md)

> 这是 parked 的设计种子,不是成品。现在只写 README,这套后续再展开。
...
- **Brick 2** DailyAgent 最小切片 —— 每天一次生成日程 → ticker 驱动 current_activity。= demo 切片("她有自己的一天")。
...
1. CC 只读 dump `character_states` 现状:current_activity/mood 读回 prompt 没?有无规则演化?proactive 怎么消费?→ 决定 Brick 1 是"补"还是"从头",也决定 README #3 能说多满。
2. 展开成 `docs/design/character-mechanism.md`(大框架 + 四层 + brick spec)。本身是 PM 作品集料。
