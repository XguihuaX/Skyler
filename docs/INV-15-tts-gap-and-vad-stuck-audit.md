...
## §9 Ship 记录 · P2 VAD stream stale recovery 三件套(2026-05-27)

PM 真机后 sharp 观察:"VAD 时好时不好 · 后续才不好" → 99% MediaStream stale + 无
recovery。PM 拍板 ship Option G+H+I 复合 · 一 commit 闭环。

### §9.1 Commit

| Commit | 主题 | 文件 | LoC |
|---|---|---|---|
| `8537393` | **P2 三件套** · mic stream stale recovery + diagnostic UI | 3(`useAudio.ts` + `VadBar.tsx` + `store/index.ts`)| +242 / -18 |

### §9.2 三件套细节

**Option H · `initStream` 健康检查**(核心 fix):
- 拆 `createAudioGraph` / `teardownAudioGraph` / `isAudioGraphHealthy` 三 helper
- 复用 stream 前 verify `MediaStreamTrack.readyState === 'live'` AND
  `AudioContext.state !== 'closed'`
- 不健康 → teardown + 重建
- AudioContext 'suspended' → try resume() 优先 · 失败再重建
- 周期兜底:vadLoop 每 60 帧(~1s)再检查一次 · onended 漏 fire 时兜底

**Option I · `MediaStreamTrack.onended` listener**(自动 recovery):
- `createAudioGraph` 每个 track 挂 `addEventListener('ended', ...)`
- ended 触发 → `recoverStream('track-ended')`:
  - vadState=sleep · 不偷偷重申 mic · teardown 等下次 toggleVad
  - 非 sleep · stop RAF → teardown → createAudioGraph → restart RAF
  - `recoveryInFlightRef` 单飞行锁 · 防 race(onended + 周期 check 同时触发)
  - permission denied → `setVadState('sleep')` 让用户感知

**Option G · VadBar 实时 max + threshold readout**(diagnostic):
- store 加 `vadCurrentMax: number` + setter
- vadLoop 每帧写本字段(60fps · Zustand set 轻量)
- `toggleVad → sleep` 清零防陈旧数字残留
- VadBar 从纯彩条 → progress bar + threshold marker + "now: X / threshold: Y" 数字
- 等宽字体 tabular-nums 让数字位稳定
- max ≥ threshold 时换 accent 色(语义"该录音了")
- title attr 提示"数字不动 = mic 流可能 stale"

### §9.3 三件套互动 paths

```
正常 path:
  toggleVad → initStream → createAudioGraph → MediaRecorder 触发条件:max ≥ threshold

stale path(系统切 mic / Tauri suspend / 权限 revoke):
  track.onended → recoverStream('track-ended')
    → [if vadState != sleep] teardown + createAudioGraph + restart RAF → 恢复

onended 漏 fire 兜底 path(Tauri webview / 浏览器 idiosyncrasy):
  vadLoop frame N % 60 == 0 → check tracks readyState
    → [if not live] recoverStream('periodic-check') → 同上恢复

诊断 path(PM 看 UI · 不依赖 console.log):
  vadCurrentMax 数字动 + < threshold → 阈值高(slider 调 65→50)
  vadCurrentMax 数字 stuck 0 → stream stale · 等 ≤ 1s 周期 check 自动 recover
    or reload tab(最快 · 但通常不需要)
```

### §9.4 Sanity(CC 侧 dev-time)

- `npx tsc -p tsconfig.app.json --noEmit` clean
- 0 regression risk:三件套全是 additive(新 helper / 新 listener / 新 store
  字段)· 老路径行为不变

预期真机(PM 测试场景):
- **场景 A**:正常 chat 10 min 无系统级 mic 干扰 — VAD 全程 work · 数字 max 动
- **场景 B**:切窗口去 IDE · 切 Cursor 再切回来 — 若 Tauri webview suspend ·
  trigger onended · 看 console "stream recovered" log · 数字恢复动
- **场景 C**:系统抢 mic(eg 微信视频/Zoom 用 mic)再切回来 — track ended 自动
  recover · 数字恢复
- **场景 D**:WS 频繁重连(独立 issue · 单独 audit backlog)— VAD recovery 与 WS
  独立 · 不互相影响

### §9.5 PM 调整 vadThreshold 65→50(Option K · 不需 ship)

PM 直接 UI slider 调 · 0 code · 立即生效(localStorage persist)。结合 P2 后 ·
threshold + VAD recovery 互补:
- threshold 50 → 正常说话(120-180 max)可压过 raw 127 · 录音触发更敏感
- recovery 三件套 → 即便系统/浏览器干扰 · 自动恢复 stream · 不需 reload

### §9.6 ⚠️ 限制 + 未来 backlog

**已 cover**:
- ✓ system mic 切源 / 应用后台 / Tauri suspend → recover
- ✓ AudioContext suspended → resume
- ✓ onended 漏 fire → 周期检查兜底
- ✓ recovery race → 单飞行锁
- ✓ 诊断 UI 让 PM 自助看是 stream 问题 vs 阈值问题

**未 cover** (P3 backlog · 若 PM 真机后仍有罕见 case):
- 浏览器 page visibility hidden 时(eg switch tab)· 可加 `visibilitychange`
  监听 · 后台时主动 sleep · 前台时自动 resume
- mic permission 长期被拒 · 持续 recover 失败 · 加用户提示 modal(不只 vadState=sleep)
- 多 mic 设备切换(默认设备改变)· 当前 recovery 用 `audio: true` 拿默认 ·
  老 stream stop 后新 stream 拿可能换设备 · 需测

### §9.7 配套 Lesson #22

`docs/LESSONS.md` 加 Lesson #22:VAD / mic 等实时 input 必须三件套(stream 健
康检查 / onended recovery / diagnostic UI)· 缺一不可。

### §9.8 ROADMAP 同步

`ROADMAP.md` P2 "**ASR whisper preload + 入口可达性**" 等条目 · 加 INV-15 P2
reference(虽 INV-15 主线是 TTS gap · P2 三件套是 VAD-specific 修复 · 与 INV-14
P1 ASR fix 互补但不依赖)。

---

**§9 ship 闭环 · P2 三件套单 commit · stream stale recovery + 诊断 UI 配套
ship · 等 PM 真机回归 · 若 1 周内无 "VAD 卡空闲" 复现 · INV-15 整段 closed**。
