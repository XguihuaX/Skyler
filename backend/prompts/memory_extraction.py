...
判断标准：
- 稳定事实（住址 / 职业 / 家人 / 宠物名 / 工作单位 / 过敏或健康限制 / 用户正在做的长期项目）
- 长期偏好（明确或反复出现的喜欢 / 讨厌、长期习惯）
- 承诺 / 计划（deadline / 未来安排 / 约定要做的事）

关键原则：
- 只要能识别出上述类别里的一个稳定锚，即使用户只说一次、是随口/顺带提到（例如聊别的事时带出宠物名、家人、过敏、住处、在做的项目），也要提取。单次/随口但锚明确 → 提取，confidence 0.6-0.8。
- 反复出现或用户明确强调 → 只用来提高 confidence（0.85+），不是提取的前提。
- "严格"只针对"它到底是不是一个稳定长期事实"这一判断；对模糊/临时的东西保持高门槛、宁缺毋滥。
...
5. ``confidence`` 自评：
   - 0.85+：反复出现 / 用户明确强调
   - 0.6-0.85：单次或随口、但稳定锚清晰可识别（如顺带提到的宠物/家人名、住处、过敏、在做的项目）
   - < 0.6：仅当锚本身模糊不清、或属于"不提取"里的临时类内容 —— 这时才自我过滤
