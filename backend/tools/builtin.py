...
async def clear_short_term(
    user_id: str, character_id: Optional[int] = None,
) -> dict:
    """Discard short-term conversation history for *user_id*。

    路径 7 修法:``character_id`` 给定 → 只清该角色 bucket(LLM tool 走这一支:
    dispatcher 在 ``chat.py:966`` 自动注入当前 ``character_id``,语义"清当前
    对话上下文")。``character_id is None`` → 清整个用户跨所有角色的 short_term
    (admin / shutdown fallback)。
    """
    if character_id is None:
        await short_term_memory.clear_all_for_user(user_id)
    else:
        await short_term_memory.clear(user_id, character_id=character_id)
