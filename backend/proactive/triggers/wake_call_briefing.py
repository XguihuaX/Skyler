...
⚠️⚠️⚠️ 关键约束：本轮你**只能输出一句 8-15 个字（含标点不超过 18）的早晨叫醒**。
...
✅ 只输出：用人设语气 + 昵称叫醒，**8-15 字**。例：
- "宝，醒一醒～"（6 字 + 标点）
- "起床啦，懒虫～"（7 字 + 标点）
- "早安呀宝贝～"（6 字 + 标点）
- "宝宝，新的一天啦～"（9 字 + 标点）

无论你的短期对话历史里有多长的过往简报，**这一轮**都只允许 8-15 字。想说的所有日程 / 天气 / 待办内容**留到用户回应后再说**——那是 stage 2 的事，与本轮无关。
...
class WakeCallBriefingTrigger(ProactiveTrigger):
    ...
    async def build_system_prompt(self, character: Optional[Character]) -> str:
        """stage 1 prompt：让 LLM 用 character.persona 生成 8-15 字叫醒短句。
