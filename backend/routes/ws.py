...
async def _handle_message(
    ...
    with timed("Total turn"):
        ...
        # 读取当前 character 的 voice_model 和 response_language
        voice_model: Optional[str] = None
        resp_lang: str = "zh"  # A2:LLM 输出语种,默 zh
        if char_id is not None:
            try:
                async with AsyncSessionLocal() as session:
                    row = (await session.execute(
                        select(Character.voice_model, Character.response_language)
                        .where(Character.id == char_id)
                    )).one_or_none()
                    if row is not None:
                        voice_model = row[0] if isinstance(row[0], str) else None
                        resp_lang = row[1] if isinstance(row[1], str) and row[1].strip() else "zh"
            except Exception:
                logger.exception(
                    "Failed to load character fields for id=%s", char_id,
                )
        ...
        # A2 翻译辅助 — 关闭 ws / tts_engine。每轮定义一次,task 异步执行不阻塞
        # producer(translate + synth 与下一句 LLM 流水线并行)。
        from backend.tts.translate import translate_for_tts

        async def _translate_synth(
            ...
            if synth is None:
                try:
                    ...
                    pass
                return None
            return await _tts_synth_with_timeout(tts_engine, synth, emotion, idx=idx)
                ...
                async for sentence in _agent_stream:
                    ...
                    if get_tts_enabled():
                        if tts_language == resp_lang:
                            ...
                            task = asyncio.create_task(
                                _translate_synth(
                                    final_chunk, tts_language, char_id,
                                    turn_emotion, sentence_idx,
                                )
                            )
