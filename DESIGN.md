...
## 十四之A、Live2D 架构（v3-E1 完成后定型）

v3-E1 Step 1-6 走完后，Live2D 整体架构按"模型无关层 + 模型相关层"两段式分离。任何后续模型替换、emotion / motion 扩展都按这个分层来。

### 模型无关层（核心 pipeline，跨模型不变）

不论使用哪个 Live2D 模型（Hiyori / 八重神子 / 自制 Momo / ...），以下数据流都保持不变：
...
- **`Live2DCanvas` useEffect 监听**：currentEmotion → emotion 视觉绑定；currentMotion → `model.motion(group, index, NORMAL)`

### 模型相关层（per-character 配置）

每个 character 绑定一个 Live2D 模型，配置：

- **`emotionMap`** —— 中文/英文 emotion 名 → expression 文件名 / 参数偏移列表
- **`motionMap`** —— 中文 motion 名 → `{ group, index }`，触发 `model.motion()`
- **`hitAreaMap`** —— hit area name → 触摸响应行为（v3-E1 Step 3 当前是 canvas 整体点击 + 随机 Tap motion；v3-E2 可扩展为 hit-test 细分）

v3-E1 当前 motionMap / emotionMap 写在 `frontend/src/config/live2d.ts` **全局共享**，v3-E2 必须升级为 **per-character 配置**（每个角色独立 map）—— 见 ROADMAP v3-E2 清单。

### 当前 Hiyori 限制（v3-E1 实测后定型）

- **无 `.exp3.json` 文件** → emotion 视觉绑定 deferred to v3-E3（数据流已就绪，绑定空实现）
- **4 个 Flick* motion group**（语义已通过 dev `__triggerMotion` 调试钩子实测对齐）：
  - `Flick` —— 放松甩手（慵懒 / 随意）
  - `FlickDown` —— 双手别身后（害羞 / 收敛）
  - `FlickUp` —— 小臂举起晃（加油 / 应援）
  - `Flick@Body` —— 复合动作 + 表情（撒娇 / 俏皮）
- **没有"挥手 / 点头 / 鞠躬 / 打招呼"等具体语义动作** —— 在 `_build_motion_instruction` 已显式告诉 LLM 这些词不可用
- **idle / Tap / Tap@Body 资源分配** —— 见 ROADMAP v3-E1 实测表

---

## 十四之B、Tech Debt（characters 双源真相）

**当前现状（v3-E1 角色修复 `ba2efd2` 后）**：

DB `characters` 表负责 UI 切角色 + per-character 隔离（chat_history / memory / voice_model / live2d_model）；`config/characters.yaml` 负责 prompt_manager + `switch_character` LLM tool。`_build_messages` 优先从 DB 拿 persona，失败 / 没 character_id 才回 YAML —— 这是**方案 B**。

**张力点**：UI 切角色走 DB id 命中先；LLM tool 切角色改 prompt_manager state 但前端仍带原 character_id，DB persona 仍命中先 —— LLM tool 路径事实上失效。

**方案 C 彻底统一**（v3-G 后期或 v4 处理）：
